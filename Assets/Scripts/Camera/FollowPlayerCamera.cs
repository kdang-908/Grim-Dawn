...
public class FollowPlayerCamera : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // nếu chưa gán target sẵn trong Inspector thì tìm player luôn 1 lần
        if (target == null)
            TryFindTarget();
    ...
    void Update()
    {
        // nếu vì lý do gì đó target bị mất (vd player chết, spawn lại) → thử tìm lại
        if (target == null)
            TryFindTarget();
    ...
    void TryFindTarget()
    {
        ...
        Debug.Log("[FollowPlayerCamera] Bind to: " + target.name);
    }
